...
export function applyBallDeceleration(velocity: Position): Position {
  ...
  newVelocity.x *= 0.9985; // Almost no deceleration - billiard physics
  newVelocity.y *= 0.9985; // Almost no deceleration - billiard physics
  ...
  if (newSpeed < 6.0) {
    ...
    const factor = 6.0 / Math.max(0.01, newSpeed); // Prevent division by zero
