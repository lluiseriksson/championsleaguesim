...
// Handle collisions between ball and goalkeepers
export function handleGoalkeeperCollisions(
  ...
  const goalkeeperCollisionCooldown = 100; // ms
  ...
  for (const player of fieldPlayers) {
    ...
    if (collision) {
      ...
      if (distance > 0) {
        ...
        if (ownGoalRisk) {
          // More aggressive prevention of own goals
          ...
          // Billiard-style collision with high elasticity
          newVelocity.x += adjustedDx * 2.0; 
          newVelocity.y += normalizedDy * 2.0;
        }
      }
